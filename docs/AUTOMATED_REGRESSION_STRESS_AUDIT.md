...
- Added deterministic pending-flow watchdog tests for pending mutator choice/action/second-action ownership, stale/invalid payload rejection, and owner-only resolution behavior.
- Added deterministic watchdog assertions for RPS/parry and manual coin-flip pending resolution idempotency, including duplicate/late submission safety and pending-state clear-once behavior.
- H3B intentionally defers broader endgame idempotency race coverage (H3C), bounded reconnect/bot churn loops (H3D), and optional seeded stress harness work (H3E).
