...
async function handleMove(io, socket, gameManager, data) {
  ...
  if (isBoardMove) {
    ...
    const halfMove = parseInt(parts[4]) + 1;
    ...
    const correctedFen = fenParts[0] + ' ' + newTurn + ' ' + parts[2] + ' - ' + halfMove + ' ' + fullMove;
  ...
  if (room.mutatorState) {
    ...
    for (const ar of ms.activeRules) {
      executeHook(ar.rule.id, 'onAfterMove', room, playerColor, { from, to }, captured);
    }

    // Fire onCapture hooks if a piece was captured
    if (captured) {
      for (const ar of ms.activeRules) {
        executeHook(ar.rule.id, 'onCapture', room, playerColor, captured, to);
      }
    }
