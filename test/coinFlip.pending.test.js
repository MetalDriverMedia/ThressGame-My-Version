...
test('triggerCoinFlip does not run while pendingAction, pendingSecondAction, or pendingRPS is unresolved', () => {
  ...
  assert.equal(room.mutatorState.pendingCoinFlip, null);
  assert.equal(roomEvents.some((e) => e.name === 'coinFlipPrompt' || e.name === 'coinFlip' || e.name === 'coinFlipResult'), false);
  assert.deepEqual(room.mutatorState.pendingSecondAction, {
    ruleId: 'drafted',
    actionType: 'square',
    forPlayer: 'b',
    firstChoiceData: 'e4',
